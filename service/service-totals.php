...
try {
    ...
    $sql = "SELECT COUNT( * ) AS  `Visits` FROM BookVisits WHERE  `DateViewed` >  '06/01/2017' AND  `DateViewed` <  '06/31/2017'";
        $tot = $pdo->prepare($sql);
        $tot->execute();
        $total = $tot->fetch(PDO::FETCH_ASSOC);
        $servtotal1 = json_encode($total);
        
    $sql2 = "SELECT COUNT( * ) AS  `To Do` FROM EmployeeToDo WHERE  `DateBy` >  '2017-06-01*' AND  `DateBy` <  '2017-06-31*'";
        $tot2 = $pdo->prepare($sql2);
        $tot2->execute();
        $total2 = $tot2->fetch(PDO::FETCH_ASSOC);
        $servtotal2 = json_encode($total2);

    $sql3 = "SELECT COUNT( * ) AS  `Messages` FROM EmployeeMessages WHERE  `MessageDate` >  '2017-06-01*' AND  `MessageDate` <  '2017-06-31*'";
    ...
    $sql5 = "SELECT COUNT( DISTINCT  `CountryCode` ) AS `Countries` FROM BookVisits WHERE  `DateViewed` >  '06/01/2017' AND  `DateViewed` <  '06/31/2017'";
        $tot5 = $pdo->prepare($sql5);
        $tot5->execute();
        $total5 = $tot5->fetch(PDO::FETCH_ASSOC);
        $servtotal5 = json_encode($total5);
